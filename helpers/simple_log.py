...
simple_log = lambda *args, **kwargs: print(f'{Fore.RESET}{datetime.now().strftime(f'%Y-%m-%d{kwargs['sep'] if 'sep' in kwargs else ' '}%H:%M:%S:%f')}{f'{kwargs['sep'] if 'sep' in kwargs else ' '}[{Fore.BLUE}{kwargs['name'] if 'name' in kwargs else ODOO_MODULE_NAME}{Fore.RESET}]' if (ODOO_MODULE_NAME != None or 'name' in kwargs) and not ('name' in kwargs and kwargs['name'] == None) else ''}{kwargs['sep'] if 'sep' in kwargs else ' '}[{kwargs['level'] if 'level' in kwargs else f'{Fore.GREEN}INFO{Fore.RESET}'}]', *((args[0:-1] + (str(args[-1]) + '.',)) if not (type(args[-1]) == str and args[-1][-1] == '.') else args), **{key: kwargs[key] for key in kwargs if key not in ['name', 'level']})